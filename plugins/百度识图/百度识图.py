name = '百度识图'
# from sdk import Bot

st_list = list()
qqbot = object
gp_id = 0

import json
import os
import re
import time

import requests
import requests_toolbelt


# pic_path = 'test.png'


def getRandomKey():
    url = 'https://miao.baidu.com/abdr'
    requestdata = ''
    res = requests.post(url=url, data=requestdata).content.decode('utf-8')
    return res


def uploadPic(key: str, picpath: str):
    url = 'https://graph.baidu.com/upload?uptime=' + str(time.time())
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36',
        'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundary9z1M4HX8R3wt8diF'}
    multipart = requests_toolbelt.MultipartEncoder(
        fields={
            'image': (os.path.basename(picpath), open(picpath, 'rb')),
            'tn': 'pc',
            'from': 'pc',
            'image_source': 'PC_UPLOAD_FILE',
            'sdkParams': key
        },
        boundary='----WebKitFormBoundary9z1M4HX8R3wt8diF'
    )
    dic = requests.post(url, data=multipart.to_string(), headers=headers).json()
    sign = dic["data"]["sign"]
    return "https://graph.baidu.com/s?sign=" + sign, sign


def getMoreSizePic(sign):
    '''获取更多尺寸图片，没有的话返回空字典'''
    if sign is None:
        raise "No sign"
    url = "https://graph.baidu.com/s?sign=" + sign + "&f=all"
    ret = requests.get(url).text
    it = re.finditer("var cardData = (.+?);\n", ret)
    for match in it:
        dic = json.loads(match.group(1))
        if dic["extData"]["template_name"] == "atom/imageSize":
            url = dic["tplData"]["moreSizeUrl"]
            break
    else:
        return {}
    ret = requests.get(url)
    searchresult = re.search('ajaxUrl.:.(.+?)"', ret, re.M | re.I)
    if searchresult:
        url = searchresult.group(1)
    else:
        return {}
    url = url.replace('\\', "")
    ret = requests.get(url)
    return json.loads(ret)


def getSimilarPic(sign: str, page=1):
    '''获取相似图片'''
    if sign == None:
        raise "No sign"
    url = "https://graph.baidu.com/view/similardetailnew?sign=" + sign + \
          "&f=refinement&srcp=crs_simi&render_type=json&carousel=1&next=1&tn=wise&idctag=gz&page=" + \
          str(page)
    ret = requests.get(url).json()
    return ret


# result = getMoreSizePic()

def down_pic(picurl):
    res = requests.get(picurl).content
    with open('./data/百度识图/' + str(hash(str(picurl))), 'wb') as f:
        f.write(res)


def init(Api: object, p_id: int):  # 框架启动调用
    try:
        os.mkdir('./data/百度识图')
    except:
        pass
    global gp_id
    global qqbot
    gp_id = p_id
    qqbot = Api
    return {'name': name, 'complain': '茉莉云AI对话', 'author': 'huazi', 'ver': '0.1', 'p_id': p_id,
            'respond_group_msg': True, 'respond_private_msg': True, 'respond_notice': False, 'respond_request': True}


def close():
    pass


def enable():  # 插件被启用时被调用
    pass


def disable():
    pass


def on_group_msg(dic: dict):
    if dic['message'] == '百度识图' and dic['user_id'] not in st_list:
        st_list.append(dic['user_id'])
        qqbot.send_group_msg(dic['group_id'], '请发送图片')
    elif dic['user_id'] in st_list:
        st_list.remove(dic['user_id'])
        pic_url_ = re.findall(r',url=(.*?);', dic['message'])
        down_pic(pic_url_[0])
        pic_path = './data/百度识图/' + pic_url_[0]
        url, sign = uploadPic(getRandomKey(), pic_path)
        s = getMoreSizePic(sign)
        if s == {}:
            s = getSimilarPic(sign)
            piclist = "list"
            picwidth = "thumbWidth"
            picheight = "thumbHeight"
            picurltype = 1  # "thumbUrl"
        else:
            piclist = "albumList"
            picwidth = "width"
            picheight = "height"
            picurltype = 0  # "thumbUrl"

        if s["status"] != 0:
            qqbot.send_group_msg(dic['group_id'], '错误,status不为0')
            # print('status不为0')
        else:
            # print('下载图片中')
            result = s["data"][piclist]
            # tmp = ""
            n = 0
            # pic_url=''
            for x in result:
                n += 1
                if picurltype == 1:
                    picurl = x["thumbUrl"]
                else:
                    picurl = x["imgList"][0]["imgSrc"]
                # picpath = "C:\\imageCache\\" + str(time.time()) + ".jpg"
            qqbot.send_group_msg(dic['group_id'], ''.join(picurl))
        os.remove('./data/百度识图/' + str(hash(pic_url_[0])))
        # print(picurl)


def on_private_msg(dic: dict):
    if dic['message'] == '百度识图' and dic['user_id'] not in st_list:
        st_list.append(dic['user_id'])
        qqbot.send_msg({'msg_type': 'private', 'number': dic['user_id'], 'msg': '请发送图片'})
    elif dic['user_id'] in st_list:
        st_list.remove(dic['user_id'])
        pic_url = re.findall(r',url=(.*?);', dic['message'])
        down_pic(pic_url)
        pic_path = './data/百度识图/' + str(hash(pic_url[0]))
        url, sign = uploadPic(getRandomKey(), pic_path)
        s = getMoreSizePic(sign)
        if s == {}:
            s = getSimilarPic(sign)
            piclist = "list"
            picwidth = "thumbWidth"
            picheight = "thumbHeight"
            picurltype = 1  # "thumbUrl"
        else:
            piclist = "albumList"
            picwidth = "width"
            picheight = "height"
            picurltype = 0  # "thumbUrl"

        if s["status"] != 0:
            qqbot.send_msg({'msg_type': 'private', 'number': dic['user_id'], 'msg': '错误'})
            # print('status不为0')
        else:
            # print('下载图片中')
            result = s["data"][piclist]
            tmp = ""
            n = 0
            for x in result:
                n += 1
                if picurltype == 1:
                    picurl = x["thumbUrl"]
                else:
                    picurl = x["imgList"][0]["imgSrc"]
                # picpath = "C:\\imageCache\\" + str(time.time()) + ".jpg"
                qqbot.send_msg({'msg_type': 'private', 'number': dic['user_id'], 'msg': '\n'.join(picurl)})
    os.remove('./data/百度识图/' + str(hash(pic_url[0])))
    # qqbot.send_group_msg(dic['group_id'], '\n'.join(picurl))
    # print(picurl)


def on_notice(dic):
    pass


def setting():
    pass
